...
def call( String reportsFolder="reports" ) {
    ...
    print "Container Vulnerbility scan (${branch}/${image}:${tag})"
    print "============================= ${branch}/${image}:${tag} image scanning...  =============================="
    print "============================= Vulnerbility scan report  ==========================================="
      sh """
            trivy --severity HIGH,CRITICAL --no-progress --ignore-unfixed ${branch}/${image}:${tag} | tee './${image}:${tag}.report'
         """
